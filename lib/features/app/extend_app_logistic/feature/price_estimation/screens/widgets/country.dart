...
class Country extends StatefulWidget {
  ...
  Country({required this.transport_id});
  @override
...
class _CountryState extends State<Country> {
  ...
  void initState() {
    ...
    super.initState();
    _blocController.add(FetchTargetCountryList());
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<PriceEstimationBloc, PriceEstimationState>(
        ...
        builder: (context, state) {
          if (state is FetchingTargetCountry) {
            return Center(child: CircularProgressIndicator());
          }
          if (state is ErrorFetchingTargetCountry) {
            return Center(child: Text(state.error.toString()));
          }
          if (state is FetchedTargetCountry) {
            if (selectedItem.text == "Select Country") {
              selectedItem.text = state.result[0].name.toString();
            }
            return Container(
              ...
              child: DropdownButton<TargetCountryModel>(
